...
def boggle_words(board, minlength=3):
    ...
    def extend_boggle(pref, path):

        if pref in WORDS and len(pref) >= minlength:
            results.add(pref)
        
        if pref in PREFIXES:
            for n in neighbors(path[-1], N):
                if n not in path and board[n] != BORDER:
                    extend_boggle(pref+board[n],path + [n])            
    
    results = set()
    N = size(board)
    for idx, sq in enumerate(board):
        if sq != BORDER:
            extend_boggle('', [idx])    
    return results
...
||||||""".strip()
    ...
    assert len(boggle_words(Board('TPLER ORAIS METND DASEU NOWRB'))) == 312
